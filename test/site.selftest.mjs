...
check("README and canonical agent doc give an honest current integration path", /## Agent developers/.test(readme) && /docs\/AGENT\.md/.test(readme) && /shade-tree-\$VERSION-\$TARGET-live/.test(readme) && /shade-tree enroll/.test(readme) && /shade-tree proxy-token/.test(readme) && /SHADE_TREE_PROXY_TOKEN/.test(readme) && !/shade-tree proxy[^\n]*--secret/.test(readme) && /shade-tree run --proxy[^\n]*your-agent/.test(readme) && /no repo-maintained public v4\s+connection profile yet/i.test(readme) && /shade-tree-\$VERSION-\$TARGET-live/.test(agentGuide) && /There is no repo-maintained public v4 access profile/.test(agentGuide) && /shade-tree enroll/.test(agentGuide) && /shade-tree proxy-token/.test(agentGuide) && /SHADE_TREE_PROXY_TOKEN/.test(agentGuide) && !/shade-tree proxy[^\n]*--secret/.test(agentGuide) && /shade-tree run -- hermes/.test(agentGuide));
