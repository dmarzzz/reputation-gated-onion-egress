...
check("Grove composes a compact mobile hero and lowers the desktop globe", /\.network-visual\s*\{[^}]*transform:\s*translateY\(clamp\(1\.5rem, 3vh, 2\.75rem\)\);/.test(groveCss) && /@media \(max-width: 760px\)[\s\S]*?\.network-copy h1\s*\{[^}]*max-width:\s*none;[^}]*font-size:\s*clamp\(2\.75rem, 8vw, 4rem\);[^}]*line-height:\s*0\.9;[^}]*\}[\s\S]*?\.network-copy p\s*\{[^}]*font-size:\s*0\.8rem;[^}]*line-height:\s*1\.5;[^}]*\}[\s\S]*?\.network-stage\s*\{[^}]*width:\s*min\(76vw, 29rem\);[^}]*\}[\s\S]*?\.network-visual\s*\{[^}]*transform:\s*none;[^}]*\}/.test(groveCss) && /@media \(max-width: 480px\)[\s\S]*?\.network-copy h1\s*\{[^}]*font-size:\s*clamp\(2\.25rem, 10\.5vw, 2\.75rem\);[^}]*\}[\s\S]*?\.network-stage\s*\{[^}]*width:\s*min\(78vw, 20\.5rem\);[^}]*\}/.test(groveCss));
