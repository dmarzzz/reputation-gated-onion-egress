...
check("Grove ends after the live network hero without a secondary analytics panel", /<main id="grove-main">\s*<header class="network-hero"[\s\S]*?<\/header>\s*<\/main>\s*<footer class="site-footer">/.test(grovePage) && !/24 hours of shade|history-panel|history-chart|history-summary|relay-ledger|onchain-ledger/.test(grovePage));
check("Grove keeps the announced node count in its signed snapshot detail table", /<aside class="provenance-panel" aria-label="Snapshot provenance">[\s\S]*?<span>Snapshot<\/span>[\s\S]*?<strong data-snapshot-state>Verifying<\/strong>[\s\S]*?<div class="provenance-count"><dt>Announced nodes<\/dt><dd data-node-count>…<\/dd><\/div>\s*<div><dt>Network<\/dt><dd data-network>…<\/dd><\/div>\s*<div><dt>Scope<\/dt><dd>Pre-v4 research fleet<\/dd><\/div>\s*<div><dt>Observed<\/dt><dd data-view-time>…<\/dd><\/div>\s*<div><dt>Refresh target<\/dt><dd data-snapshot-cadence>15 min<\/dd><\/div>[\s\S]*?<\/aside>/.test(grovePage) && (grovePage.match(/<aside class="provenance-panel"/g) || []).length === 1 && (grovePage.match(/<dt>/g) || []).length === 5 && !/network-data|data-view-age|data-node-hours|24h node-hours|A view with boundaries|Signed view|Observer attests|Browser verifies|A scheduled observer|<dt>Definition<\/dt>|Announced within TTL/.test(grovePage) && !/\.network-data/.test(groveCss));
...
check("Grove dashboard stays inside the signed aggregate contract", ["data-node-count", "data-snapshot-state", "data-network", "data-view-time", "data-snapshot-cadence"].every((field) => grovePage.includes(field)) && !/data-history-|data-relay-|data-onchain-|data-view-age|data-node-hours|data-change|Observed changes?|sessions|throughput|latency|bandwidth|success rate|reputation band/i.test(grovePage));
